...
2026년 1월부터 7월까지의 KOSPI 시계열 데이터를 수집하고 분석하기 위한 작업 공간입니다. 현재 단계에서는 분석 코드를 작성하지 않고 다음과 같이 기본 구조만 준비했습니다.
...
원본 데이터는 `data/raw`에 보관하고, 정제·변환을 마친 데이터는 `data/processed`에 저장합니다. 분석 과정은 `notebooks`에서 기록하고, 최종 시각화 결과는 `images`에 저장합니다. 데이터 파일과 생성 이미지는 Git에서 제외됩니다.
